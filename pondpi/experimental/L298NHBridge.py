...
leftmotorpwm = io.PWM(leftmotorpwm_pin, 50)
rightmotorpwm = io.PWM(rightmotorpwm_pin, 50)
...
def setMotorLeft(power):
    ...
    if power < 0:
        ...
        set_motor_mode("leftmotor", "reverse")
        pwm = -int(PWM_MAX * power)
        if pwm > PWM_MAX:
            pwm = PWM_MAX
    elif power > 0:
        # Forward mode for the left motor
        set_motor_mode("leftmotor", "forward")
        pwm = int(PWM_MAX * power)
        if pwm > PWM_MAX:
            pwm = PWM_MAX
    else:
        # Stopp mode for the left motor
        set_motor_mode("leftmotor", "stopp")
        pwm = 0
...
def setMotorRight(power):
    ...
    if power < 0:
        ...
        set_motor_mode("rightmotor", "reverse")
        pwm = -int(PWM_MAX * power)
        if pwm > PWM_MAX:
            pwm = PWM_MAX
    elif power > 0:
        # Forward mode for the right motor
        set_motor_mode("rightmotor", "forward")
        pwm = int(PWM_MAX * power)
        if pwm > PWM_MAX:
            pwm = PWM_MAX
    else:
        # Stopp mode for the right motor
        set_motor_mode("rightmotor", "stopp")
        pwm = 0
